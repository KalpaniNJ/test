...
def show(params):
    ...
    with tab2:
        ...
        with col1:
            st.markdown("#### Input Dependency")
            st.info("Requires output from Time Series Analysis.")
            if "ts_df_points" not in st.session_state:
                st.warning("Please run Time Series Analysis first.")
            else:
                st.success("Time Series data found.")
    ...
    with tab3:
        ...
        with col1:
            st.markdown("#### Input Parameters")
            dates = params.get("season_dates", {})
            st.write(f"Start: {dates.get('start', '–')}")
            st.write(f"Peak: {dates.get('peak', '–')}")
            st.write(f"Harvest: {dates.get('harvest', '–')}")
    ...
    with tab4:
        ...
        with col1:
            st.markdown("#### Input Dependency")
            st.info("Requires completed Rice Mapping output.")
